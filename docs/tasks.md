...
- **Notas 2026-05-11:** La hoja de personaje abierto fue reorganizada en secciones Figma con métricas superiores, estado opcional de penalización en sigilo, XP, imagen/atributos, magia y PG separados por dividers rojos degradados. Se corrigió la cascada para que los dividers sean visibles, se fijó el orden/label abreviado `FUE`, `DES`, `CON` / `INT`, `SAB`, `CAR`, se ajustó `Agregar experiencia` a botón mini y se alineó el padding interno a 16px X / 32px Y. Falta QA visual contra los estados con/sin penalización.
- **Status:** `[~]`
