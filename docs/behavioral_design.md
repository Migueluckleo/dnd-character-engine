...
- US146.2b: El header de personaje abierto usa composición de tres columnas: `Atrás` a la izquierda, eyebrow `Personaje` y nombre centrado, y botón mini `Tirar dado` a la derecha. La biografía es una card parchment independiente debajo de la ficha principal, con título `Biografía`, párrafo introductorio y bullets para `Ideal`, `Vínculo` y `Defecto`.
