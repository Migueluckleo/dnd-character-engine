...
- US146.2a: La card de personaje abierto se divide internamente con dividers rojos degradados visibles: métricas superiores, estado opcional de penalización en sigilo, XP, imagen/atributos, magia y PG. Los estados con y sin penalización deben mantener la misma arquitectura, cambiando solo la línea contextual de penalización. La grilla de atributos debe leerse `FUE`, `DES`, `CON` / `INT`, `SAB`, `CAR`; el botón `Agregar experiencia` es mini de 24px alto con `Source Serif Pro` 10px regular y padding horizontal de 8px.
