...
- The opened-character sheet card must use the Figma section architecture: XP row with level badge/progress/action first, then metric row (`CA`, `Velocidad`, `B. Competencia`), optional stealth-penalty copy when equipped gear imposes `stealth_disadvantage`, image + 3x2 ability grid, magic 3-column row, hit-point 2-column row, and red gradient/taper dividers between major sections.
- The opened-character header must follow the prototype composition: back action on the left, centered eyebrow `Personaje`, centered character name below it, and mini `Tirar dado` action on the right.
- The opened-character biography must render as a separate parchment card below the main sheet card, not as a nested section inside the main stats card. It must show an introductory paragraph followed by bullets for `Ideal`, `Vínculo`, and `Defecto` when those values exist, and it must only be visible in the `Personaje` tab.
- New Style bottom navigation bars must use `16px` horizontal padding. Their internal grid columns must fit inside the padded content width instead of using fixed columns that consume the full viewport width.
- The XP row, metric row, and optional stealth-penalty copy must be grouped in code as `.character-card-stats`, implementing the Figma `--module-xp` anatomy so its internal spacing, level badge, XP progress, metrics and penalty state remain independent from the opened-character card's global section gap.
- The opened-character ability grid must always render in this visual order: `FUE`, `DES`, `CON` on the first row and `INT`, `SAB`, `CAR` on the second row. Red sheet dividers must remain visible even when generic divider styles exist elsewhere.
- The opened-character ability grid must use the abbreviated labels literally, not full ability names. The `Agregar experiencia` control in that sheet must use the mini button spec: 24px height, `Source Serif Pro` 10px regular, and 8px horizontal padding.
- The opened-character inner card spacing must use 16px horizontal padding, 32px top/bottom padding, and 16px vertical spacing before and after each red section divider.
...
- The `Mochila` section must match the Figma `--module-inventory` and `--dndCharacterEngine-character-backpack` specs: top inventory tabs, `Carga`/coins module, search row (`Buscar objeto` + `Buscar`), 16px vertical gaps, parchment item cards with strips, and the add-object CTA in the opened-character header. Opening `Inventario` should default to `Mochila`.
...
- Inventory item cards in `Equipo`, `Mochila` and `Alijo` must render collapsed by default so the player can scan the largest possible number of items. The caret expands/collapses inline details; tapping the card opens the `Descripción` modal.
...
**Inventory helper readiness note (2026-05-15):** Inventory UI must never expose raw English item names, snake_case item IDs, item types, attribute keys or blank art because `window.DND_ITEM_HELPERS` loads late. Both `src/client/inventoryHelpers.ts` and `ui.html` must normalize display names, `item_id` values and `equipment:*` refs through the same lookup key before rendering. `ui.html` must keep defensive Spanish fallbacks for critical inventory labels, filter labels and item art, and render a local red fallback icon until the modular helpers become available. When the Vite ready event arrives, visible inventory surfaces should re-render from the canonical helper module.
...
**Traceability registry note (2026-05-15):** `docs/traceability.md` and `docs/architecture_map.md` are mandatory bridge documents for Graphify and agent handoff. Any change that connects product requirements, Figma contracts, frontend renderers, backend services, preview data or validation must update the relevant traceability section with explicit US IDs, task IDs, Figma source keys, file paths, function names and validation commands. These bridge docs prevent documentation, code and visual contracts from becoming disconnected graph silos.
