...
- The opened-character biography must render as a separate parchment card below the main sheet card, not as a nested section inside the main stats card. It must show an introductory paragraph followed by bullets for `Ideal`, `Vínculo`, and `Defecto` when those values exist.
