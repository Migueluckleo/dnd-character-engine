...
- The opened-character sheet card must use the Figma section architecture: top metric row (`CA`, `Velocidad`, `B. Competencia`), optional stealth-penalty copy when equipped gear imposes `stealth_disadvantage`, XP row with level badge/progress/action, image + 3x2 ability grid, magic 3-column row, hit-point 2-column row, and red gradient/taper dividers between major sections.
