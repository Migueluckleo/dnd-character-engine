...
- The XP row, metric row, and optional stealth-penalty copy must be grouped in code as `.character-card-stats`, implementing the Figma `--module-xp` anatomy so its internal `8px` spacing, `16px` top padding, level badge, XP progress, metrics and penalty state remain independent from the opened-character card's global section gap.
