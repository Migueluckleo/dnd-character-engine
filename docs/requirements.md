...
- The XP row, metric row, and optional stealth-penalty copy must be grouped as a single `character card stats` frame in code (`.character-card-stats`) so their internal spacing is independent from the opened-character card's global section gap.
